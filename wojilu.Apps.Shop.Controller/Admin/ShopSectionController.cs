...
namespace wojilu.Web.Controller.Shop.Admin {
    ...
    public partial class ShopSectionController : ControllerBase {
        ...
        public void Create( int columnId ) {
            ShopSection section = ShopValidator.ValidateSection( columnId, ctx );
            if (errors.HasErrors) {
                run( Add, columnId );
            }
            else {
                sectionService.Insert( section );
                //echoToParent( lang( "opok" ), to( new ShopController().Index ) );
        ...
        public void AddAutoThree(int columnId)
        {
            ...
            Service service = ServiceContext.Get(serviceId);
            ShopSectionTemplate template = templateService.GetById(templateId);
            template = checkJsonTemplate(template);
            bindServiceThree(serviceId, templateId, service, template);
        }

        private ShopSectionTemplate checkJsonTemplate(ShopSectionTemplate template)
        {
        ...
        public void CreateFeed(int layoutId)
        {
            ...
            // 创建区块
            ShopSection section = ShopValidator.PopulateFeed(layoutId, ctx);
        ...
        public void Delete(int id)
        {
            ...
            echoRedirectPart(lang("opok"), to(new ShopController().Home), 0);
        }
        ...
        public void Combine(int sectionId)
        {
            ...
            List<ShopSection> sections = sectionService.GetForCombine(section);
        ...
        private void updateFeedParamValues(ShopSection section, String url, int count)
        {
