...
namespace wojilu.Web.Templates.Tokens {
    ...
    public class VarInfo {
        ...
        public static VarInfo Init( String varName ) {

            VarInfo x = new VarInfo();

            int dotIndex = varName.IndexOf( '.' );
            if (dotIndex > 0 && dotIndex < varName.Length) {

                x.setIsObject( true );

                String[] arrp = varName.Split( '.' );

                x.setObjName( arrp[0] );
                x.setObjAccessor( arrp );

            }

            // TODO
            //x.setRawLabelAndVar( this.objVar.GetFullLabelAndVar() );
            x.setRawLabelAndVar( "#{" + varName + "}" );
